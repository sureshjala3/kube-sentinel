...
This guide explains how to configure OAuth authentication for Kube Sentinel, supporting multiple authentication providers.
...
3. In the redirect URI, fill in `https://${HOST}/api/auth/callback`, replacing `${HOST}` with your Kube Sentinel deployment address.
   1. For example, if your Kube Sentinel is deployed at `kube-sentinel.example.com`, the redirect URI should be `https://kube-sentinel.example.com/api/auth/callback`.
   2. Kube Sentinel uses the backend Host and protocol from the request to generate the redirect Host by default.
   3. If kube-sentinel is deployed behind a proxy, it will read the `X-Forwarded-Host` and `X-Forwarded-Proto` headers by default.
...
By default, even after successful login, Kube Sentinel will not grant any permissions to the user. You need to manually configure RBAC rules to grant access.
...
3. View Kube Sentinel logs for more error information.
